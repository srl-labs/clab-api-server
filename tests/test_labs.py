...
def test_list_labs_includes_created(api_url, auth_headers, apiuser_lab):
    ...
    logging.info(f"[TEST] Verifying lab '{lab_name}' is in the list for the owner.")
    ...
    logging.info(f"  `-> Lab '{lab_name}' found in list.")
...
def test_inspect_created_lab(api_url, auth_headers, apiuser_lab):
    ...
    logging.info(f"[TEST] Inspecting details for lab '{lab_name}'.")
    ...
    logging.info(f"  `-> Inspection successful for '{lab_name}'.")
...
def test_create_duplicate_lab_fails(api_url, auth_headers, apiuser_lab, simple_topology_content, deploy_timeout):
    ...
    logging.info(f"[TEST] Attempting to create duplicate lab '{lab_name}' (expecting 409).")
    ...
    logging.info(f"  `-> Received status {resp.status_code}. Asserting it's 409.")
...
def test_reconfigure_lab_owner_succeeds(api_url, auth_headers, apiuser_lab, simple_topology_content, deploy_timeout, lab_stabilize_pause):
    ...
    logging.info(f"[TEST] Attempting to reconfigure owned lab '{lab_name}' (expecting 200).")
    ...
    logging.info("  `-> Pausing for stabilization after reconfigure...") # Removed f
...
def test_reconfigure_lab_non_owner_fails(api_url, apiuser_headers, superuser_lab, simple_topology_content, deploy_timeout):
    ...
    logging.info(f"[TEST] Attempting non-owner reconfigure on lab '{lab_name}' (expecting 403).")
    ...
    logging.info(f"  `-> Received status {resp.status_code}. Asserting it's 403.")
...
def test_reconfigure_lab_superuser_succeeds(api_url, superuser_headers, apiuser_lab, simple_topology_content, deploy_timeout, lab_stabilize_pause):
    ...
    logging.info(f"[TEST] Attempting superuser reconfigure on lab '{lab_name}' (expecting 200).")
    ...
    logging.info("  `-> Pausing for stabilization after superuser reconfigure...") # Removed f
    time.sleep(lab_stabilize_pause)
...
def test_list_labs_superuser(api_url, superuser_headers, apiuser_lab, superuser_lab):
    ...
    logging.info(f"[TEST] Verifying superuser sees labs '{apiuser_created_lab}' and '{superuser_created_lab}'.")
    ...
    logging.info("  `-> Superuser list check successful.") # Removed f
...
def test_list_labs_apiuser_filters(api_url, apiuser_headers, apiuser_lab, superuser_lab):
    ...
    apiuser_created_lab = apiuser_lab
    superuser_created_lab = superuser_lab
    logging.info(f"[TEST] Verifying apiuser sees '{apiuser_created_lab}' but NOT '{superuser_created_lab}'.")
    ...
    logging.info("  `-> Apiuser list filtering check successful.") # Removed f
